...
def getPassesAnalysis(op1,op2,dateFrom, dateTo,format):
    try:
        requestURL = Baseurl +'passesanalysis'+'/'+ op1 + '/' +op2 +'/'+ dateFrom+'/'+dateTo
        response = requests.get(requestURL)
        response = response.json()

        #############
        if format =='csv':
            with open(str(op1)+'_'+str(op2)+'_'+str(dateFrom) +'_' +str(dateTo)+'_'+'passesAnalysis.csv', 'w') as f:
                field_names = ['charge', 'passID', 'stationID','timestamp','vehicleID']
...
if __name__ == '__main__':
    ...
    if args.format[0] != 'csv' and args.format[0] != 'json':
        ...
        if args.op1 and args.op2 and  args.datefrom and args.dateto and args.format:
            getPassesAnalysis(args.op1[0],args.op2[0] ,args.datefrom[0],args.dateto[0],args.format[0])
        else:
